...
│   ├── llm_reviewer.py              # LLM forensic analysis with deterministic fallback
...
flowchart TB
    ...
    subgraph PROCESS [Single Case Pipeline]
        ...
        RECONCILE --> LLM["run_vllm_review()<br/>Build LLM messages with:<br/>- canonical_case<br/>- precomputed_discrepancies<br/>- raw_extracted_documents (all sheets)<br/><br/>Try LLM API → if fails → fallback_llm_review()"]
...
| **7. LLM Review** | `run_vllm_review()` | Case + discrepancies + raw docs | `LLM review dict` | Builds system prompt with forensic persona; sends canonical data + all raw sheets to LLM; parses JSON response; falls back to deterministic if LLM unavailable |
...
VLLM_BASE_URL = "https://openrouter.ai/api/v1"          # OpenRouter by default
MODEL_NAME = "qwen/qwen3-14b"                           # LLM model
VLLM_API_KEY = os.getenv("ONLINE_LLM_KEY")              # From .env file
...
### 4.10 `src/llm_reviewer.py` — LLM Forensic Review

**Purpose:** Universal LLM interface for forensic tax analysis with deterministic fallback.
...
**Client Factory:** Uses OpenAI-compatible SDK. Auto-detects provider from base URL.

```mermaid
flowchart TD
    START["run_vllm_review()"] --> BUILD["build_llm_messages()<br/>System prompt: Senior ITO persona<br/>User prompt: canonical_case + discrepancies + raw docs"]
...
> "You are a senior Indian Income Tax Officer... forensically analyse tax documents... You have access to PRE-COMPUTED DISCREPANCIES and RAW EXTRACTED DOCUMENTS (complete sheet-by-sheet data)... Output ONLY valid JSON matching the required schema."
...
### 4.11 `src/decision.py` — Decision Composer
...
### 4.12 `src/document_gen.py` — Document Generation
...
### 4.13 `src/output.py` — Output Packaging
...
|-----------|---------|---------------------|-------------|
| `VLLM_BASE_URL` | `https://openrouter.ai/api/v1` | `LLM_BASE_URL` | LLM API endpoint |
| `MODEL_NAME` | `qwen/qwen3-14b` | `LLM_MODEL` | LLM model name |
| `VLLM_API_KEY` | From env `ONLINE_LLM_KEY` | `LLM_API_KEY` | API key |
| `LIBREOFFICE_CMD` | `soffice` (win) / `libreoffice` (nix) | `LIBREOFFICE_CMD` | PDF conversion tool |
| `INPUT_DIR` | Auto-detected (`Input` or `input`) | — | Case file directory |
...
The LLM integration uses the OpenAI-compatible API format, making it compatible with virtually any modern LLM serving infrastructure.

**Supported Providers:**

| Provider | Base URL Pattern | Auth |
|----------|-----------------|------|
| OpenRouter | `https://openrouter.ai/api/v1` | API key in `ONLINE_LLM_KEY` |
| OpenAI | `https://api.openai.com/v1` | API key in `LLM_API_KEY` |
| vLLM | `http://localhost:8000/v1` | `dummy` (or configured) |
| Ollama | `http://localhost:11434/v1` | `ollama` (auto) |
| Custom | Any URL | Configurable |

**Provider Auto-Detection:**
```python
def _detect_provider(base_url: str) -> str:
    # Checks URL for keywords: openrouter, openai, localhost/8000 (vLLM), localhost/11434 (ollama)
```

### Message Construction (`build_llm_messages()`):

**System Prompt:** Persona of a senior Indian Income Tax Officer. Instructs to:
...
2. Falls back to finding outermost `{ }` boundaries
3. Validates all 4 required keys exist
4. If parsing fails → falls back to deterministic analysis
...
echo "ONLINE_LLM_KEY=your_key_here" > .env
...
### Docker (via vLLM for LLM)

```bash
# Start vLLM with Qwen3-14B
...
*Document generated from source code analysis — covers all modules, data flows, configuration, LLM integration, fallback chains, testing, and operational details of the Tax Investigation System.*
